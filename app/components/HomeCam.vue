...
export default {
  ...
  methods: {
    showCamera() {
      ...
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        navigator.mediaDevices.getUserMedia(constraints)
          ...
          .catch((error) => {
            // Handle errors (such as camera not found or permission denied)
            console.error('getUserMedia error:', error);
            alert("Camera not found or permission denied");
            // Handle the error gracefully (e.g., switch camera facing mode)
            this.toggleCameraFacingMode();
          });
      } else {
        // Fallback for browsers that don't support getUserMedia
        alert('getUserMedia is not supported in this browser');
        // Handle unsupported browser case (e.g., display a message or provide alternative)
      }
    },
    ...
    toggleCameraFacingMode() {
      ...
      this.showCamera(); // Attempt to show camera again with the updated facing mode
    },
    // showCamera() {
    //   const myvideo = document.getElementById("local_video");
    //   myvideo.srcObject = null;
